...
import { Button, Caption, Field, PhotoPlaceholder, Screen, Title, radius, space, useTheme } from './ui';
...
export default function FirstComposeSheetScreen() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (existing) {
        await Firsts.update(existing.id, { title: title.trim(), note: note.trim() || null, happenedAt, assetOwnerUserId, assetId });
      } else {
        await Firsts.create({ familyId: family?.id, title, note, happenedAt, assetOwnerUserId, assetId });
      }
      close();
    } catch (err) {
