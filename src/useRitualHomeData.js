...
function annotatePromptState(promptState, userId) {
  ...
  const answered = responses.filter((row) => !!row.response_text);
  const snoozed = !!(
    mine?.snoozed_until
  ...
  return {
    ...
    mineAnswered: !!mine?.response_text,
...
function buildDerivedPayload({ raw, userId }) {
  ...
  return {
    updatedAt: Date.now(),
    promptState,
    digest: raw.digest || WeeklyDigests.build({ photos: shared, memories, firsts, letters }),
    sharedPhotos: shared,
...
async function fetchRitualHomePayload({ familyId, userId }) {
  const [shared, memories, firsts, letters, promptState, members] = await Promise.all([
  ...
  const digest = await WeeklyDigests.ensureForCurrentWeek({
    ...
    firsts,
    letters,
  }).catch(() => WeeklyDigests.build({ photos: shared, memories, firsts, letters }));
