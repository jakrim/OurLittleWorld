...
import { Screen, Button, Brand, Hero, Body, Caption, V, H, Spacer, semantic, colors, space, radius, shadow } from './ui';
...
export default function ReferencePhotoScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (parsed?.uri) setPicked({ uri: parsed.uri });
        if (parsed?.embedding?.length) setEmbedding({ embedding: parsed.embedding, faceCount: 1 });
        if (parsed?.uri || parsed?.embedding) setRestored(true);
      } catch {}
    })();
    return () => { alive = false; };
  }, [family?.id, user?.id]);
  ...
  const onContinue = async () => {
    ...
    const key = referenceStorageKey({ familyId: family.id, userId: user.id });
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            await AsyncStorage.removeItem(referenceStorageKey({ familyId: family.id, userId: user.id }));
            setPicked(null);
            setEmbedding(null);
            setRestored(false);
            setError(null);
  ...
  return (
    <Screen variant="warm" scroll>
      ...
      <V gap="lg" style={{ paddingTop: space.sm, paddingBottom: space.xxl }}>
        ...
        <Body>
          We use it as a reference to find every other photo of them in your
          library — privately, on your device. Your library never leaves
          your phone.
        </Body>
        ...
        {restored && !error ? (
          <H gap="sm" align="center" justify="center">
            ...
            <Caption style={{ color: colors.plum, fontWeight: '700' }}>
              USING SAVED REFERENCE — TAP TO CHANGE
            </Caption>
          </H>
        ) : null}
        ...
        <Button onPress={onContinue} loading={busy} disabled={!picked}>
          {restored ? 'Continue with this reference' : 'Find every photo'}
        </Button>
