...
| User Content: Photos or Videos | Yes | Selected/saved photos in Supabase storage | Current app behavior is photo-focused. Select this category because saved family photos are uploaded to private storage. |
| User Content: Other User Content | Yes | Memory notes, daily prompt responses, firsts, letters, family profile and invite details | Text users create or save inside the private family space. |
...
- Audio Data: The app includes microphone permission text and the schema has an `audio_storage_object` column for prompt responses, but the current code scan did not find an implemented recording/upload path. Do not select Audio Data for the current build unless voice notes are actually reachable in the shipped app.
- Camera: The app includes camera permission text for capturing a reference photo. If captured reference photos are uploaded or stored remotely, include Photos or Videos as already selected above. If they remain local-only, no separate additional category is needed beyond permission disclosure.
...
- Whether voice notes are reachable in build `1.1.1`.
