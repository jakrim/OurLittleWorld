-- Base schema and storage
-- Fresh environments need these tables/functions before the ritual migrations run.
...
create index if not exists memories_family_asset_idx
  on public.memories(family_id, asset_owner_user_id, asset_id);
...
create policy family_photos_select on storage.objects for select
  using (
    bucket_id = 'family-photos'
    and exists (
      select 1 from public.family_members fm
      where fm.family_id::text = split_part(storage.objects.name, '/', 1)
        and fm.user_id = auth.uid()
    )
  );

drop policy if exists family_photos_insert on storage.objects;
create policy family_photos_insert on storage.objects for insert
  with check (
    bucket_id = 'family-photos'
    and exists (
      select 1 from public.family_members fm
...
create policy family_photos_update on storage.objects for update
  using (
    bucket_id = 'family-photos'
    and exists (
      select 1 from public.family_members fm
...
create policy family_photos_delete on storage.objects for delete
  using (
    bucket_id = 'family-photos'
    and exists (
      select 1 from public.family_members fm
