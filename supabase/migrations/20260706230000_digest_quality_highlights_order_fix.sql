-- W2 fix: assemble_weekly_digest representative_media was ordered by
-- `row_number() over ()` (empty window), whose value Postgres does NOT
-- guarantee to follow the subquery's ORDER BY. jsonb_agg(... order by rank)
-- then re-sorted by that meaningless rank, so milestone-linked and
-- higher-quality media did NOT reliably rank first.
--
-- Fix: rank explicitly with row_number() OVER (ORDER BY <criteria>) in a CTE,
-- take the top 4, then aggregate ordered by that deterministic rank. Only the
-- v_representative_media block changes from 20260706202414.
...
begin
  ...
  -- Rank first (milestone-linked, then captureQuality desc, then recency),
  -- keep the top 4, then aggregate in that exact rank order.
  with ranked as (
    select
      row_number() over (
        order by
