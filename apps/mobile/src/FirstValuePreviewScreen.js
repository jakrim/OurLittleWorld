...
export default function FirstValuePreviewScreen() {
  ...
  const approve = async () => {
    ...
    try {
      ...
      console.warn('first value approval failed');
      Alert.alert(
        'Could not save your choice yet',
        'Your photo is still private. Please try again.',
      );
    } finally {
      setBusy(false);
    }
  ...
  const keep = async () => {
    ...
    try {
      ...
      console.warn('first value keep failed');
      Alert.alert(
        'Could not keep this moment yet',
        'Nothing was duplicated. Please try again when your connection is ready.',
      );
    } finally {
      setBusy(false);
    }
