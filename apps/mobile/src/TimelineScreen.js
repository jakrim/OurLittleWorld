...
export default function TimelineScreen() {
  ...
  const babyName = family?.babyName || null;
    ...
    >
      ...
      <View style={styles.forYouLabel}>
        <View style={styles.forYouIcon}>
          <Ionicons name={entry.icon} size={12} color={colors.onPrimary} />
        </View>
...
function ScanBanner({ onPress, onAutoSavedTick }) {
  ...
  const waiting = scan.matches.reduce((n, m) => n + (!m.saved ? 1 : 0), 0);
...
function EmptyTimeline({ babyName, onAdd, onScan, onBrowse }) {
  const title = babyName ? `Start ${babyName}'s first page.` : 'Start the first page.';
  return (
    <View style={styles.empty}>
      ...
      <Spacer h={space.lg} />
      <Hero align="center" style={{ fontSize: 28 }}>{title}</Hero>
      <Spacer h={space.sm} />
      <Body align="center" style={{ paddingHorizontal: space.xl }}>
        Add one photo, voice note, or tiny memory now. Discovery can wait until
        you want the app to look for older moments.
      </Body>
      <View style={styles.emptySteps}>
        <EmptyStep icon="image-outline" title="Save one favorite" body="Pick a photo or video without granting full-library access." />
        <EmptyStep icon="mic-outline" title="Keep a voice" body="Record the sound of this week before it changes." />
        <EmptyStep icon="sparkles-outline" title="Discover later" body="Optional scans stay local first and wait for review." />
      </View>
      <Spacer h={space.xl} />
      <View style={{ width: '100%', paddingHorizontal: space.xl }}>
        <Button onPress={onAdd}>Add first moment</Button>
        <Spacer h={space.sm} />
        <Button variant="quiet" onPress={onBrowse}>Browse my library instead</Button>
        <Spacer h={space.sm} />
        <Button variant="ghost" onPress={onScan}>Start discovery scan</Button>
      </View>
    </View>
  );
}

function EmptyStep({ icon, title, body }) {
  return (
    <View style={styles.emptyStep}>
      ...
      <View style={styles.emptyStepText}>
        <Caption style={styles.emptyStepTitle}>{title}</Caption>
    ...
    <Pressable style={styles.browseTile} onPress={onPress}>
      ...
      {age ? (
        <View style={styles.browseAge}>
          <Caption style={{ color: colors.onPrimary, fontSize: 10, lineHeight: 12, fontWeight: '700' }}>{age}</Caption>
        </View>
      ) : null}
      {tagged ? (
