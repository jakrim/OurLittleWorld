...
<head>
  ...
  <title>${escapeHtml(child)} - Our Little World Family Archive</title>
  <style>
    :root {
      color-scheme: light;
      --paper: #fffaf4;
...
<body>
  <main>
    <header>
      <div class="eyebrow">Our Little World family archive</div>
      <h1>${escapeHtml(child)}</h1>
      <p class="lede">${escapeHtml(EXPORT_POLICY_COPY.alwaysExportable)} ${escapeHtml(EXPORT_POLICY_COPY.lapsedVault)} ${escapeHtml(EXPORT_POLICY_COPY.exportScope)} This parent-approved archive was generated from saved family memories on ${escapeHtml(generated)}.</p>
      <div class="stats">
    ...
    ${renderAnnotationSection(annotations, annotationAuthors)}
    ${renderCollectionSection(collections)}
    ${renderVoiceSection(voiceItems, stats)}
    ${renderLimitationsSection(limitations)}
    <p class="note">This file is generated locally by the app. Image links come from the current private archive session and should be exported again before printing or sharing broadly.</p>
  </main>
</body>
</html>`;
}
...
function renderVoiceSection(voiceItems, stats) {
  ...
  const items = voiceItems.length
    ? voiceItems
    : [{ title: 'Voice notes', body: `${number(stats?.voiceNotes)} voice notes are saved in the family archive.` }];
  return renderNoteSection('Voice', 'Voice references', items);
}

function renderAnnotationSection(annotations, authors) {
  const rows = (annotations || []).filter(Boolean).map((annotation) => {
    const author = authors?.[annotation.author_user_id] || 'A parent';
    return {
      title: annotation.annotation_type === 'voice' ? `${author} added a voice note` : `${author} added context`,
      body: annotation.annotation_type === 'voice'
        ? `Voice contribution saved on ${formatDate(annotation.created_at)}.`
        : [annotation.body, formatDate(annotation.created_at)].filter(Boolean).join(' - '),
    };
  });
  if (!rows.length) return renderLimitedSection('Parent context', 'Parent contributions', 'No separately authored parent context is available yet.');
  return renderNoteSection('Parent context', 'Parent contributions', rows);
}

function renderCollectionSection(collections) {
  const rows = (collections || []).filter((collection) => Number(collection?.moment_count || 0) > 0).map((collection) => ({
    title: collection.title || 'Saved collection',
    body: `${number(collection.moment_count)} ${Number(collection.moment_count) === 1 ? 'memory' : 'memories'} · ${collection.confidence_band || 'factual'} source`,
  }));
  if (!rows.length) return renderLimitedSection('Collections', 'Automatic collections', 'No factual collections are available yet.');
  return renderNoteSection('Collections', 'Automatic collections', rows);
}
...
function renderNoteSection(eyebrow, title, rows) {
  ...
  <p class="muted">${escapeHtml(row.body || 'Saved with the family archive.')}</p>
</article>`).join('\n');
  return `<section>
  <div class="eyebrow">${escapeHtml(eyebrow)}</div>
  <h2>${escapeHtml(title)}</h2>
