...
import { Screen, Card, Button, Field, GlassButton, PhotoPlaceholder, Title, Body, Caption, Eyebrow, V, H, Spacer, glass, space, radius, useTheme } from './ui';
...
export default function PhotoDetailScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const myMem = mems.find((m) => m.author_user_id === userId);
        ...
        if (alive) {
          setAsset({
            ...
            downloadError: String(err?.message || err || 'Could not load photo details'),
          });
        }
      } finally {
  ...
  const displayUri = useMemo(
    () => renderableUri(asset?.localUri) || renderableUri(previewUri) || renderableUri(asset?.uri) || null,
    [asset?.localUri, asset?.uri, previewUri],
  );

  const assetDownloadFailed = asset?.downloadStatus === 'failed';
  ...
  const placeLabel = useMemo(() => {
    ...
    return 'Place saved from photo metadata';
    ...
    <Screen scroll keyboard>
      <V gap="lg" style={{ paddingTop: space.md, paddingBottom: space.xxl }}>
        <View style={styles.imageWrap}>
          ...
          {(assetLoading || assetDownloadFailed || (!displayUri && asset?.downloadStatus === 'pending')) ? (
            <PhotoDownloadState
              loading={assetLoading}
              failed={assetDownloadFailed}
              error={asset?.downloadError}
              onRetry={() => setAssetReloadKey((value) => value + 1)}
              theme={theme}
            />
          ) : null}
...
function renderableUri(uri) {
  const value = String(uri || '');
  if (!value) return null;
  if (value.startsWith('ph://')) return null;
  return value;
}

function PhotoDownloadState({ loading, failed, error, onRetry, theme }) {
  return (
    <View style={styles.downloadOverlay}>
      <View style={[styles.downloadCard, { backgroundColor: theme.semantic.card, borderColor: theme.semantic.border }]}>
        {loading ? (
          <ActivityIndicator color={theme.semantic.primary} />
        ) : (
          <Ionicons name="cloud-offline-outline" size={22} color={theme.semantic.primary} />
        )}
        <View style={styles.downloadCopy}>
          <Body style={styles.downloadTitle}>
            {loading ? 'Downloading from iCloud' : failed ? 'Photo not available yet' : 'Waiting for photo download'}
          </Body>
          <Caption numberOfLines={2}>
            {failed
              ? (error || 'Open Photos or retry after the original finishes downloading.')
              : 'The private archive will keep the preview here while the original loads.'}
          </Caption>
        </View>
        {!loading ? (
          <Pressable
            onPress={onRetry}
            hitSlop={8}
            style={[styles.retryButton, { backgroundColor: theme.colors.primarySoft }]}
          >
            <Caption style={[styles.retryLabel, { color: theme.semantic.primary }]}>Retry</Caption>
          </Pressable>
        ) : null}
      </View>
    </View>
